...
@Component({
  selector: 'app-directimcome',
  templateUrl: './directimcome.component.html',
  styleUrls: ['./directimcome.component.css']
})
export class DirectimcomeComponent implements OnInit {
  pairincome:any;
  uid:any;
  searchTerm:any='';
    ...
    ngOnInit(): void {
      this.afa.user.subscribe(data => {
        ...
        this.http.get<any>('http://moneysagaconsultancy.com/api/api/totaluserdata?user_id='+this.uid).subscribe(response => {
          this.pairincome = response.diectincome;
